...
export default async function DisputePage({ params, searchParams }: { params: Promise<{ id: string }>; searchParams: Promise<Params> }) {
  ...
  return <HistoryShell session={session}><Link href="/history/disputes">← All disputes</Link><HistoryHead title="One message. Two versions.">Compare the stored and incoming text, and see who supplied each version.</HistoryHead>
    ...
    </div></section> : <p className="help"><Link href="/settings#advanced-mode">Enable Advanced mode in Settings</Link> to resolve this dispute. You can keep the stored version, accept an incoming version, or delete the archived message.</p>}</div><aside className="card history-aside"><h3>Why this was flagged</h3><p>The incoming message had the same identity but different text, without an explicit edit timestamp. Steno kept the stored version.</p><p><Link href={`/history/sources/${d.source.id}`}>View source history →</Link></p>{candidate && <p>Seen {candidate.occurrences} {candidate.occurrences === 1 ? 'time' : 'times'}. First received {candidate.firstSeenAt.toISOString()}.</p>}</aside></div>
  </HistoryShell>
}
