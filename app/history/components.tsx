...
import { historyOptions, historyPage, formatHistoryCounts, HISTORY_KINDS, OPERATIONS, type HistoryFilters, type Operation } from '@/lib/services/history'
...
export function HistoryFiltersForm({ params, source }: { params: Params; source?: string }) {
  ...
  return <form className="history-filters" method="get">
    ...
    <button type="submit">Apply</button><Link href={source ? `/history/sources/${source}` : '/history'}>Clear</Link>
  </form>
}
export function HistoryFeed({ filters, params, base = '/history' }: { filters: HistoryFilters; params: Params; base?: string }) {
  ...
  return <section className="history-record" aria-label="Recorded activity"><div className="history-record-head"><span>Newest first</span><span>Times in UTC</span></div>
    {page.events.length === 0 ? <div className="empty"><h2>No matching activity</h2><p>Try another filter, or push data to start recording activity.</p></div> : page.events.map(e => <article className="history-event" key={e.id}>
      <time dateTime={e.occurredAt.toISOString()}><span>{e.occurredAt.toISOString().slice(0, 10)}</span><span>{e.occurredAt.toISOString().slice(11, 16)}</span></time>
      <div className="history-event-main"><p><span className="history-kind">{e.kind}</span> <strong>{OPERATIONS[e.operation as Operation] ?? 'Activity'}</strong></p>
        {e.subjectLabel && <p className="help">Key: {e.subjectLabel}</p>}<p className="help">{e.sources.map((s, i) => <span key={s.sourceId}>{i > 0 && ' · '}<Link href={`/history/sources/${s.sourceId}`}>{s.label}</Link></span>)}{e.sources.length > 0 && ' · '}{e.surface}</p>
        <p className="help">{formatHistoryCounts(e.counts)}</p>
        {e.kind === 'push' && (e.counts.edited || e.counts.conflicts) ? <p className="help">Edits and conflicts are included in duplicates.</p> : null}
      </div><div className="history-event-side"><span>{e.actorLabel}</span><span className={e.outcome === 'failed' || e.counts.conflicts ? 'history-warning' : ''}>{e.outcome}</span>{e.kind === 'sync' && e.finishedAt && <span>{Math.max(0, Math.round((e.finishedAt.getTime() - e.occurredAt.getTime()) / 1000))} seconds{e.operation === 'live' ? ' · five-minute bucket' : ''}</span>}{!!e.counts.conflicts && <Link href={historyHref('/history/disputes', {}, { source: e.sources[0]?.sourceId })}>Review disputes →</Link>}</div>
    </article>)}
