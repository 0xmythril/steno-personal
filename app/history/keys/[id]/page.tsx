...
export default async function PurgePage({ params, searchParams }: { params: Promise<{ id: string }>; searchParams: Promise<Params> }) {
  ...
  return <HistoryShell session={session}><Link href="/history/keys">← Revoked keys</Link><HistoryHead title={`Data from ${preview.key.label}`}>This key has been revoked. It can no longer read or push.</HistoryHead>
    ...
    <div className="history-summary"><div><p className="eyebrow">Remaining messages</p><strong>{n}</strong><p className="help">Originally delivered by this key</p></div><div><p className="eyebrow">Disputed copies</p><strong>{copies}</strong><p className="help">Pending text to remove</p></div></div>
    ...
    </section> : <p className="help"><Link href="/settings#advanced-mode">Enable Advanced mode in Settings</Link> to remove this key’s contributions. Reviewing this data does not change your archive.</p>}</HistoryShell>
}
