...
import { callTool, listTools, pushRpc } from './helpers/mcp'
import { mintAccessKey } from '@/lib/services/access-keys'
import { DATA_NOT_INSTRUCTIONS, MEDIA_URL_NOTE, NO_CONNECTION, PERSON_NOTE, SOURCE_NOTE } from '@/lib/mcp/copy'
...
async function pushKey(): Promise<string> {
  const r = await mintAccessKey('structure-push', { read: false, push: true })
  if (!r.ok) throw new Error(r.reason)
  return r.rawKey
}

async function pushTools(): Promise<{ name: string; description?: string; annotations?: { readOnlyHint?: boolean } }[]> {
  const { message } = await pushRpc(await pushKey(), { jsonrpc: '2.0', id: 1, method: 'tools/list' })
...
describe('M3 structural invariants', () => {
  ...
  it.each(['app/mcp/route.ts', 'app/mcp/push/route.ts'])(
    'every registerTool( description in %s is built from the DATA_NOT_INSTRUCTIONS constant',
    file => {
  ...
  it('list_chats, get_messages, recent_messages and search_messages describe pushers, pushedBy and source_id with the shared SOURCE_NOTE', async () => {
    const tools = await listTools(await agentKey())
    const withSource = tools.filter(t => ['list_chats', 'get_messages', 'recent_messages', 'search_messages'].includes(t.name))
    expect(withSource).toHaveLength(4)
    for (const tool of withSource) {
      expect(tool.description ?? '', `${tool.name} description`).toContain(SOURCE_NOTE)
    }
  })

  it('the push tool is not annotated read-only; every read tool still is', async () => {
    for (const tool of await listTools(await agentKey())) {
      expect(tool.annotations?.readOnlyHint, tool.name).toBe(true)
    }
    const [push] = await pushTools()
    expect(push.name).toBe('push_messages')
    expect(push.annotations?.readOnlyHint).toBe(false)
  })
  ...
  it.each(['app/mcp/route.ts', 'app/mcp/push/route.ts'])('%s never touches the account identifier column', file => {
  ...
  it.each(['app/mcp/route.ts', 'app/mcp/push/route.ts'])('%s delegates to a service, never SQL or a table import directly', file => {
  ...
  it.each(['app/mcp/route.ts', 'app/mcp/push/route.ts'])('%s authenticates with access keys only, never the portal session', file => {
    // Both MCP doors are bearer-token APIs for agents, not a browser session.
    // Importing lib/auth.ts would mean a cookie could authenticate a tool call.
    const src = readFileSync(file, 'utf8')
    expect(src).not.toMatch(/lib\/auth/)
  })

  it.each(['app/mcp/route.ts', 'app/mcp/push/route.ts'])('%s exports POST and nothing else', file => {
