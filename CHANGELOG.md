...
- **The address book fills itself in.** After every contact sync, each contact and each direct-chat counterparty with a name becomes a person automatically. A Telegram identity and a WhatsApp identity are joined into one person only when their phone numbers are equal; an identical name is still only a suggestion. A name you type is an alias and no sync overwrites it — "Use channel name" hands it back — and hiding a person keeps their links, so they are never recreated.
